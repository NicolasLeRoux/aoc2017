...
describe.only('Day 19: Tools,', function() {
	...
	describe('The method to serialize the path aka \'serializePath\',', function() {
		it('should correctly serialize the path.', function () {
			var position = {
					x: 5,
					y: 0,
					dir: 'S'
			...
			expect(utils.serializePath(matrix, position)).to.equal(['|', '|', 'A', '|', '|', '+',
				'B', '-', '+', '|', '-', '|', '+', '-', '-', '+', 'C', '|', '|', '+', '-', '-',
				'+', 'D', '+', '-', '-', '|', 'E', '-', '-', '-', '-', '|', '-', '-', '-', 'F']);
		});
	});
